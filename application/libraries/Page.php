...
class Page {
	...
	private $_header		= 'header_tpl';		// page header
	private $_left_content	= 'left_content_tpl';	// page left content
	private $_content		= '';							// page content
	private $_template		= 'index';					// default template	
	...
	public function __construct() {
		...
		if ($this->_CI->uri->segment(2) == 'pdf' OR $this->_CI->uri->segment(3) == 'pdf') {
			...
			elseif($this->_CI->session->userdata('logged_in') != '') {
	...
	function view($content = 'templates/main_content', $data = array()){
		...
		$this->_set_data();
		$this->_CI->load->view($this->_template, $this->_data);
	}
